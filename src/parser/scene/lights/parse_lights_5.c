...
static t_bool	parse_lights_direction_array_antinorm(t_rt *r, cJSON *temp,
					int i)
{
	cJSON	*elem;
	cJSON	*temp2;
	...
	if (!elem)
		return (false);
	temp2 = cJSON_GetObjectItem(elem, "direction");
	if (!temp2)
		return (false);
...
t_bool			parse_lights_direction_array(t_rt *r, cJSON *temp, int i)
{
	...
	while (i < n)
	{
		if (!(parse_lights_direction_array_antinorm(r, temp, i)))
			return (false);
		i++;
	}
	return (true);
...
static t_bool	parse_lights_direction_antinorm(t_rt *r, cJSON *pt, int i)
{
	cJSON	*temp;
	...
	{
		if (!(parse_lights_direction_single(r, temp, i)))
			return (false);
	}
	else if (temp->type == cJSON_Array)
	{
		if (!(parse_lights_direction_array(r, temp, i)))
			return (false);
	}
	else
		return (false);
...
t_bool			parse_lights_direction(t_rt *r)
{
	...
	{
		if (!(parse_lights_direction_antinorm(r, pt, i)))
			return (false);
	}
	cJSON_Delete(pt);
	return (true);
